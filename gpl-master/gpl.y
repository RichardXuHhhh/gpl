...
int yylex()
{
	...
	if(letter())
	{
		...
		else
		{
			...
			if(!flag){
				...
				char *tempName=malloc(strlen(token+1));
				strcpy(tempName,token);
				newstbNode->name=tempName;
	...
	if(digit())
	{
		...
		while(constantIntTemp->next!=NULL){
	...
	if(character=='"')
	{
		...
		while(constantStrTemp->next!=NULL){
