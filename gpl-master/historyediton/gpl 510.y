...
typedef struct identifierStruct
{
	...
	int encode;//编码
	char name[100];//存放字面量的指针
	struct identifierStruct *next;
	int num;//存放链表长度
}IDTIFIER;

typedef struct constantIntStruct
{
	
	int encode;//编码
	int *p;//存放常数值的指针
	struct constantIntStruct *next;
	char name[100];
	int num;//存放链表长度
	
}CINT;

typedef struct constantStrStruct
{
	
	int encode;//编码
	char name[100];//存放字面量的指针
	struct constantStrStruct *next;
	int num;//存放链表长度
...
int line1=1;
int startEncode=400; //常数、标识符、常字符串都从200开始编码
int count=0;//统计出现的单词个数
FILE *yyout;//用来存放输出二元组的txt文件指针
FILE *Symbol; //用来存储符号表的文件指针

CINT *constantIntList; //常数链表首节点
CSTR *constantStrList; //常字符串链表首节点
IDTIFIER *identierList; //标识符首节点
STB *symbolTableList; //标识全体符号表链表首节点
STB *lastNode; //存放symbolTablelist最后一次节点
int *tempInt;
CINT *constantIntTemp; //临时指向链表中某个节点
CSTR *constantStrTemp; 
IDTIFIER *identierTemp;
...
void error1(char c)
{
	fprintf(yyout,"nearly line:%d character %c can not be recognized!\n", line,c);
	fclose(Symbol);
	fclose(yyout);
	exit(0);
} 

void error2(char c1,char c2)
{
	fprintf(yyout,"error:nearly line:%d character %c can not be combined by %c!\n", line,c1,c2);
	fclose(Symbol);
	fclose(yyout);
	exit(0);
} 
...
int main(int argc,   char *argv[])
{
	...
	line=1;
	//printf("alloc 1---------------->\n");
	symbolTableList=malloc(sizeof(struct symbolTableStruct));
	symbolTableList->next=NULL;
	symbolTableList->num=0;
	lastNode=symbolTableList;
	constantIntList=malloc(sizeof(struct constantIntStruct));
	constantIntList->next=NULL;
	//printf("alloc 2---------------->\n");
	constantStrList=malloc(sizeof(struct constantStrStruct));
	constantStrList->next=NULL;
	//printf("alloc 3---------------->\n");
	identierList=malloc(sizeof(struct identifierStruct));
	identierList->next=NULL;
	//printf("alloc 4---------------->\n");
	
	constantIntList->num=0;
	constantStrList->num=0;
	identierList->num=0;
...
int keyword()
{
	...
	while(strcmp(keywords[i].name,""))
	{
		if(!strcmp(keywords[i].name,token)){
			count++;
			fprintf(yyout,"word-%d-(keyword): %s (%d,0)\n", count,token,keywords[i].id);
...
int yylex()
{
	...
	if(letter())
	{
		...
		else
		{
			...
			int loop;
			int flag=0;
			identierTemp=identierList;
			for(loop=0;loop<identierList->num;loop++){
				identierTemp=identierTemp->next;
				if(!strcmp(identierTemp->name,token)){
					fprintf(yyout,"word-%d-(identifier): %s (%d,%p)\n", count,token,identierTemp->encode,identierTemp->name);
					flag=1;
					break;
					}
				}
			...
			if(!flag){
				IDTIFIER *newIdentifer=(struct identifierStruct *)malloc(sizeof(struct identifierStruct));
				STB *newstbNode=(struct symbolTableStruct*)malloc(sizeof(struct symbolTableStruct));
				newstbNode->encode=startEncode;
				newstbNode->next=NULL;
				strcpy(newstbNode->name,token);
				strcpy(newstbNode->type,"IDENTIFIER");
				lastNode->next=newstbNode;
				lastNode=newstbNode;
				fprintf(Symbol,"(%s,%s,%d)\n",newstbNode->type,newstbNode->name,newstbNode->encode);
				symbolTableList->num++;
				newIdentifer->encode=startEncode;
				startEncode++;
				strcpy(newIdentifer->name,token);
				newIdentifer->next=NULL;
				
				identierTemp->next=newIdentifer;
				identierList->num++;
				fprintf(yyout,"fisrst:word-%d-(identifier): %s (%d,%p)\n", count,token,newIdentifer->encode,newIdentifer->name);
				}
	...
	if(digit())
	{
		...
		int loop;
		int flag=0;
		constantIntTemp=constantIntList;
		for(loop=0;loop<constantIntList->num;loop++){
			constantIntTemp=constantIntTemp->next;
			
			if(!strcmp(constantIntTemp->name,token)){
				fprintf(yyout,"word-%d-(constantInt): %s (%d,%p)\n", count,token,constantIntTemp->encode,constantIntTemp->p);
				flag=1;
				break;
				}
			}
		...
		if(!flag){
			CINT *newInt=(struct constantIntStruct *)malloc(sizeof(struct constantIntStruct));
			newInt->encode=startEncode;

			STB *newstbNode=(struct symbolTableStruct*)malloc(sizeof(struct symbolTableStruct));
			newstbNode->encode=startEncode;
			newstbNode->next=NULL;
			strcpy(newstbNode->name,token);
			strcpy(newstbNode->type,"CONSTANTINT");
			lastNode->next=newstbNode;
			lastNode=newstbNode;
			fprintf(Symbol,"(%s,%s,%d)\n",newstbNode->type,newstbNode->name,newstbNode->encode);
			symbolTableList->num++;
			startEncode++;
			strcpy(newInt->name,token);
			tempInt=malloc(sizeof(int));
			int temp=atoi(token);
			newInt->p=&temp;
			newstbNode->p=&temp;
			newInt->next=NULL;
			constantIntTemp->next=newInt;
			constantIntList->num++;
			fprintf(yyout,"fisrst:word-%d-(constantInt): %s (%d,%p)\n", count,token,newInt->encode,newInt->p);
			}			
	...
	if(character=='"')
	{
		...
		int loop;
		int flag=0;
		constantStrTemp=constantStrList;
		for(loop=0;loop<constantStrList->num;loop++){
			constantStrTemp=constantStrTemp->next;
			if(!strcmp(constantStrTemp->name,token)){
				fprintf(yyout,"fisrst:word-%d-(constantStr): %s (%d,%p)\n", count,token,constantStrTemp->encode,constantStrTemp->name);
				flag=1;
				break;
				}
			}
		...
		if(!flag){
			CSTR *newConstantStr=(struct constantStrStruct *)malloc(sizeof(struct constantStrStruct));
			newConstantStr->encode=startEncode;

			STB *newstbNode=(struct symbolTableStruct*)malloc(sizeof(struct symbolTableStruct));
			newstbNode->encode=startEncode;
			newstbNode->next=NULL;
			strcpy(newstbNode->name,token);
			strcpy(newstbNode->type,"CONSTANTSTRING");
			lastNode->next=newstbNode;
			lastNode=newstbNode;
			fprintf(Symbol,"(%s,%s,%d)\n",newstbNode->type,newstbNode->name,newstbNode->encode);
			symbolTableList->num++;
			startEncode++;
			strcpy(newConstantStr->name,token);
			newConstantStr->next=NULL;
			constantStrTemp->next=newConstantStr;
			constantStrList->num++;
			fprintf(yyout,"word-%d-(constantStr): %s (%d,%p)\n", count,token,newConstantStr->encode,newConstantStr->name);
			}
