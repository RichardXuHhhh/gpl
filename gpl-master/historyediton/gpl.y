...
int main(int argc,   char *argv[])
{
	...
	cg();
	fclose(yyout);
	FILE *SymbolTable=fopen("symbol.txt","w");
	fprintf(SymbolTable,"IDENTIFIER\n");

	while(identierList->next!=NULL){
		identierList=identierList->next;
		fprintf(SymbolTable,"(%s,%d,%p)-->",identierList->name,identierList->encode,identierList->name);
		identierList=identierList->next;
		}
	fprintf(SymbolTable,"\n\n\n");
	fprintf(SymbolTable,"CONSTANTINT\n");

	while(constantIntList->next!=NULL){
		constantIntList=constantIntList->next;
		fprintf(SymbolTable,"(%d,%d,%p)-->",*constantIntList->p,constantIntList->encode,constantIntList->p);
		constantIntList=constantIntList->next;
		}
	fprintf(SymbolTable,"\n\n\n");
	
	fprintf(SymbolTable,"CONSTANTINTSTRING\n");
	
		while(constantStrList->next!=NULL){
			constantStrList=constantStrList->next;
			fprintf(SymbolTable,"(%s,%d,%p)-->",constantStrList->name,constantStrList->encode,constantStrList->name);
			constantStrList=constantStrList->next;
			}
	fclose(SymbolTable);
